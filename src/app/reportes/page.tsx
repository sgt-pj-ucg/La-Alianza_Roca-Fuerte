...
import { resolveContributorName } from "@/lib/contributor";
...
export default function ReportesPage() {
  ...
  const model = useMemo(() => {
    ...
    for (const transaction of list(data?.statement?.bank_transactions)) {
      // La edición manual es prioritaria; si no existe, se obtiene el nombre
      // desde la glosa original, por ejemplo "Traspaso De: Jorge ...".
      const member = resolveContributorName(transaction.display_name, transaction.description);
      if (number(transaction.credit_clp) > 0) for (const entry of transactionEntries(transaction, "income") as any[]) {
  ...
  return <AuthGate><main className="reports-shell"><a href="/" className="reports-back"><span>←</span> Volver al panel</a><header className="reports-header"><div><p className="eyebrow">REPORTABILIDAD FINANCIERA</p><h1>Informes de tesorería</h1><p>Indicadores oficiales, trazables y listos para apoyar la gestión mensual.</p></div><img src="/logo-la-alianza.png" alt="La Alianza Roca Fuerte"/></header>
    ...
    {loading ? <p className="reports-state">Preparando cifras oficiales…</p> : error ? <p className="reports-error">{error}</p> : !monthClosed ? <p className="reports-error">Este período no está cerrado. No se emiten reportes oficiales hasta completar y cerrar el mes.</p> : <><section className="official-band"><span>✓</span><div><b>Informe oficial · {MONTHS[month - 1]} 2026</b><p>Fuente: cartola conciliada, clasificaciones confirmadas y cierre mensual protegido.</p></div><button onClick={() => void load(month)}>Actualizar cifras</button></section>
      ...
      <section className="reports-table-card"><div className="reports-table-head"><div><p className="eyebrow">MIEMBROS Y APORTANTES</p><h2>Listado de miembros por mes</h2><p>La tesorera puede editar un nombre; si no lo hace, se identifica automáticamente desde transferencias “Traspaso De: …”. Los depósitos sin nombre bancario siguen requiriendo revisión.</p></div><button onClick={() => downloadWorkbook("members")}>↓ Exportar Excel</button></div><ReportTable headings={["Nombre", "Diezmos", "Ofrendas", "Misiones", "Aportes", "Total"]} rows={model.members.map(row => [row.name, clp(row.tithe), clp(row.offering), clp(row.missions), clp(row.aporte), clp(row.total)])} empty="No hay aportantes identificados en este período." /></section>
...
function InteractiveBars({ rows }: { rows: Breakdown[] }) {
  ...
  return <div className="ranking"><div className="bar-chart">{rows.map((row, index) => <button className={index === selected ? "bar-row selected" : "bar-row"} key={row.name} onClick={() => setSelected(index)} onMouseEnter={() => setSelected(index)} onFocus={() => setSelected(index)}><div><span>{row.name}</span><b>{clp(row.amount)}</b></div><i><em style={{ width: `${row.amount / max * 100}%` }}/></i></button>)}</div><aside><span>Selección</span><b>{current.name}</b><strong>{clp(current.amount)}</strong><small>{Math.round(current.amount / rows.reduce((sum, row) => sum + row.amount, 0) * 100)}% del total mostrado</small></aside></div>;
}
function BudgetBars({ rows }: { rows: { item: string; amount: number; budget: number }[] }) {
  ...
  if (!usable.length) return null; return <div className="budget-bars"><b>Partidas con ejecución</b>{usable.map((row, index) => <button key={row.item} onClick={() => setSelected(index)} onMouseEnter={() => setSelected(index)} onFocus={() => setSelected(index)} className={index === selected ? "selected" : ""}><span>{row.item}</span><i><em style={{ width: `${Math.min(row.budget ? row.amount / row.budget * 100 : 100, 100)}%` }}/></i><span className="budget-bar-value">{clp(row.amount)}</span></button>)}<small>{current.item}: {clp(current.amount)} ejecutado de {clp(current.budget)} presupuestado.</small></div>;
}
