...
const menu = ["Resumen", "Presupuesto", "Cartolas", "Clasificación", "Ingresos", "Diezmos y aportantes", "Egresos", "Cierre mensual", "Comparativo", "Reportes", "Configuración"];
...
export default function Home() {
  ...
  const model = useMemo(() => {
    ...
    for (const tx of transactions) {
      ...
      if (tx.credit_clp) {
        ...
        entries.forEach(entry => { const contributor = resolveContributorName(tx.display_name, tx.description); income.set(entry.name, (income.get(entry.name) ?? 0) + entry.amount); incomeDetails.set(entry.name, [...(incomeDetails.get(entry.name) ?? []), { bookedAt: tx.booked_at, description: entry.description ?? tx.display_name ?? tx.description, contributor, amount: entry.amount }]); if (["Diezmo", "Aporte", "Misiones", "Ofrenda"].includes(entry.name)) contributors.push({ ...entry, bookedAt: tx.booked_at, contributor: contributor ?? "Sin nombre asignado" }); });
  ...
  function navigate(name: string) { if (name === "Cartolas") return window.location.assign("/cartolas"); if (name === "Clasificación") return window.location.assign("/clasificacion"); if (name === "Comparativo") return window.location.assign("/comparativo"); if (name === "Reportes") return window.location.assign("/reportes"); setActive(name); setNotice(""); setError(""); }
  ...
  return <AuthGate><main className="app-shell"><aside className="sidebar"><div className="sidebar-profile"><i>MV</i><span>Miriam Véliz Cortés<small>Tesorería</small></span></div><div className="brand"><div>TESORERÍA<small>Iglesia · 2026</small></div></div><nav>{menu.map(name => <button key={name} className={active === name ? "active" : ""} onClick={() => navigate(name)}>{name}</button>)}</nav><div className="sidebar-foot">Datos financieros en CLP<br/><span>Gestión mensual</span></div></aside>
    <section className="workspace"><header><div><p className="eyebrow">GESTIÓN FINANCIERA</p><h1>{active}</h1><p className="subhead">Resumen basado en los movimientos confirmados de la cartola.</p></div><img className="header-logo" src="/logo-la-alianza.png" alt="La Alianza Roca Fuerte"/></header>
      ...
      {loading ? <p className="report-state">Actualizando información financiera…</p> : error ? <p className="classification-error">{error}</p> : <>{notice && <p className="classification-notice">✓ {notice}</p>}<p className="classification-notice">✓ Presupuesto fuente validado: {data?.integrity?.checkedEntries ?? 0} valores mensuales revisados contra el presupuesto 2026{data?.integrity?.completedEntries ? ` · ${data.integrity.completedEntries} valor(es) faltante(s) completado(s) desde la fuente.` : "."}</p>
        ...
        {active === "Diezmos y aportantes" && <section className="table-card"><div className="table-head"><h3>Diezmos y aportes</h3><span>Según las clasificaciones confirmadas</span></div><Table rows={model.contributors.map(row => [row.bookedAt, row.contributor, row.name, row.description, clp(row.amount)])} headings={["Fecha", "Aportante", "Concepto", "Descripción", "Monto"]} empty="No hay diezmos o aportes clasificados para este mes." /></section>}
