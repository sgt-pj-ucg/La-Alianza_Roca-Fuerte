...
export default function Home() {
  ...
  return <AuthGate><main className="app-shell">
    <aside className="sidebar"><div className="brand"><img src="/logo-la-alianza.png" alt="La Alianza Roca Fuerte"/><div>TESORERÍA<small>Iglesia · 2026</small></div></div><nav>{menu.map(name => <button key={name} className={active === name ? "active" : ""} onClick={() => name === "Cartolas" ? window.location.assign("/cartolas") : name === "Clasificación" ? window.location.assign("/clasificacion") : setActive(name)}>{name}</button>)}</nav><div className="sidebar-foot">Datos financieros en CLP<br/><span>Fase 2 · Cartolas</span></div></aside>
    <section className="workspace"><header><div><p className="eyebrow">GESTIÓN FINANCIERA</p><h1>{active}</h1><p className="subhead">Presupuesto 2026 importado desde la fuente original.</p></div><div className="user"><i>MV</i><span>Miriam Véliz Cortés<small>Tesorería</small></span></div></header>
