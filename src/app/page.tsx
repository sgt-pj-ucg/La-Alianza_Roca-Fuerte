...
export default function Home() {
  ...
  return <AuthGate><main className="app-shell">
    <aside className="sidebar"><div className="brand"><span>✦</span><div>TESORERÍA<small>Iglesia · 2026</small></div></div><nav>{menu.map(name => <button key={name} className={active === name ? "active" : ""} onClick={() => name === "Cartolas" ? window.location.assign("/cartolas") : name === "Clasificación" ? window.location.assign("/clasificacion") : setActive(name)}>{name}</button>)}</nav><div className="sidebar-foot">Datos financieros en CLP<br/><span>Fase 2 · Cartolas</span></div></aside>
