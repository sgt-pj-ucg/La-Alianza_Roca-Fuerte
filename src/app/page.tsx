...
export default function Home() {
  ...
  const model = useMemo(() => {
    const transactions = asList(data?.statement?.bank_transactions); const income = new Map<string, number>(); const expenses = new Map<string, number>(); const contributors: any[] = []; const expenseItems = new Map<string, number>();
    for (const tx of transactions) {
      ...
      if (tx.credit_clp) {
        ...
        entries.forEach(entry => { income.set(entry.name, (income.get(entry.name) ?? 0) + entry.amount); if (["Diezmo", "Aporte"].includes(entry.name)) contributors.push({ ...entry, bookedAt: tx.booked_at }); });
    ...
    const budgetItems: Item[] = asList(data?.budget?.budget_categories).flatMap(category => asList(category.budget_items).map((item: any) => ({ name: item.name, category: category.name, budget: Number(first(item.monthly_budgets)?.amount_clp ?? 0), executed: expenseItems.get(`${category.name}|${item.name}`) ?? 0 })));
    ...
    return { transactions, income, expenses, expenseItems, contributors, budgetItems, totalIncome, totalExpense, confirmed, pending: transactions.length - confirmed };
  }, [data]);
  ...
  return <AuthGate><main className="app-shell"><aside className="sidebar"><div className="sidebar-profile"><i>MV</i><span>Miriam Véliz Cortés<small>Tesorería</small></span></div><div className="brand"><div>TESORERÍA<small>Iglesia · 2026</small></div></div><nav>{menu.map(name => <button key={name} className={active === name ? "active" : ""} onClick={() => navigate(name)}>{name}</button>)}</nav><div className="sidebar-foot">Datos financieros en CLP<br/><span>Gestión mensual</span></div></aside>
    <section className="workspace"><header><div><p className="eyebrow">GESTIÓN FINANCIERA</p><h1>{active}</h1><p className="subhead">Resumen basado en los movimientos confirmados de la cartola.</p></div><img className="header-logo" src="/logo-la-alianza.png" alt="La Alianza Roca Fuerte"/></header>
      ...
      {loading ? <p className="report-state">Actualizando información financiera…</p> : error ? <p className="classification-error">{error}</p> : <>{notice && <p className="classification-notice">✓ {notice}</p>}
        ...
        {active === "Ingresos" && <section className="table-card"><div className="table-head"><h3>Ingresos confirmados por concepto</h3><span>Total: {clp(model.totalIncome)}</span></div><Table rows={rows.map(([name, amount]) => [name, clp(amount)])} headings={["Concepto", "Monto recibido"]} /></section>}
