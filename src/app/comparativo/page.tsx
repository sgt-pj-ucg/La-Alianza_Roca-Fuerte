...
function CumulativeChart({ records }: { records: MonthRecord[] }) {
  ...
  const max = Math.max(...records.flatMap(record => [record.cumulativeExpense, record.cumulativeBudget]), 1); const active = records[Math.min(selected, records.length - 1)];
  return <div className="cumulative-bars"><div className="cumulative-bars-legend"><span><i className="cumulative-executed-key"/> Gasto acumulado</span><span><i className="cumulative-plan-key"/> Presupuesto acumulado</span></div><div className="cumulative-bars-plot" style={{ "--periods": records.length } as CSSProperties}>{records.map((record, index) => <button type="button" key={record.month} className={index === selected ? "selected" : ""} onClick={() => setSelected(index)} onMouseEnter={() => setSelected(index)} onFocus={() => setSelected(index)} aria-label={`${record.label} 2026: ejecutado acumulado ${clp(record.cumulativeExpense)}, presupuesto acumulado ${clp(record.cumulativeBudget)}`}><span className="cumulative-bars-columns"><i className="cumulative-executed-bar" style={{ height: `${record.cumulativeExpense / max * 100}%` }}/><i className="cumulative-plan-bar" style={{ height: `${record.cumulativeBudget / max * 100}%` }}/></span><span className="cumulative-bar-tooltip"><b>{record.label} 2026 · Acumulado</b><span>Ejecutado <strong>{clp(record.cumulativeExpense)}</strong></span><span>Presupuesto <strong>{clp(record.cumulativeBudget)}</strong></span><em className={record.cumulativeExpense <= record.cumulativeBudget ? "good-text" : "alert-text"}>Disponible {clp(record.cumulativeBudget - record.cumulativeExpense)}</em></span><em>{record.label.slice(0, 3)}</em></button>)}</div><div className="chart-readout"><span>{active.label} 2026 · acumulado</span><b>Ejecutado {clp(active.cumulativeExpense)}</b><b>Plan {clp(active.cumulativeBudget)}</b><strong className={active.cumulativeExpense <= active.cumulativeBudget ? "good-text" : "alert-text"}>Disponible {clp(active.cumulativeBudget - active.cumulativeExpense)}</strong></div></div>;
}
