...
export default function ComparativoPage() {
  ...
  return <AuthGate><main className="comparison-shell">
    ...
    {loading ? <p className="comparison-state">Preparando comparativo anual…</p> : error ? <p className="comparison-error">{error}</p> : !periods.length ? <p className="comparison-error">Aún no existen meses cerrados para construir un comparativo oficial.</p> : <>
      <section className="comparison-controls"><div><b>Corte del análisis</b><span>Las cifras se actualizan al seleccionar el último mes incluido.</span></div><select value={selectedEnd} onChange={event => setSelectedEnd(number(event.target.value))}>{periods.map(period => <option key={period.period_month} value={period.period_month}>Enero – {MONTHS[number(period.period_month) - 1]} 2026</option>)}</select><button onClick={() => void load()}>↻ Actualizar</button></section>
      <section className="comparison-official"><span>✓</span><div><b>Cifras oficiales hasta {latest?.label} 2026</b><p>{model.monthsClosed} mes(es) cerrados, conciliados y clasificados. Los meses abiertos no se incorporan al cálculo.</p></div></section>
      <section className="comparison-kpis">
        <Kpi label="Gasto acumulado" value={clp(model.totalExpense)} note={`de ${clp(model.plannedToDate)} presupuestado a la fecha`} status={model.execution <= 1 ? "good" : "alert"}/>
        <Kpi label="Cumplimiento acumulado" value={pct(model.execution)} note={model.execution <= 1 ? "Dentro del presupuesto acumulado" : "Sobre el presupuesto acumulado"} status={model.execution <= 1 ? "good" : "alert"}/>
      ...
      <section className="comparison-summary"><div><p className="eyebrow">LECTURA EJECUTIVA</p><h2>Gasto contra presupuesto anual</h2><p>El gasto acumulado representa <b>{pct(model.annualExecution)}</b> del presupuesto anual, con <b>{12 - model.monthsClosed} mes(es)</b> aún fuera del cierre oficial.</p></div><div className="annual-meter" aria-label={`Ejecución anual ${pct(model.annualExecution)}`}><i style={{ width: `${Math.min(model.annualExecution * 100, 100)}%` }}/><b>{pct(model.annualExecution)}</b></div><div className="comparison-summary-value"><span>Plan anual</span><b>{clp(model.annualBudget)}</b></div><div className="comparison-summary-value"><span>Desviación acumulada</span><b className={model.totalExpense <= model.plannedToDate ? "good-text" : "alert-text"}>{clp(model.plannedToDate - model.totalExpense)}</b></div></section>
...
function MonthlyBars({ records }: { records: MonthRecord[] }) {
  ...
  return <div className="monthly-bars"><div className="month-bars-list">{records.map((record, index) => <button className={index === selected ? "selected" : ""} key={record.month} onClick={() => setSelected(index)}><span>{record.label.slice(0, 3)}</span><i><em className="monthly-plan" style={{ width: `${record.budget / max * 100}%` }}/><em className="monthly-actual" style={{ width: `${record.expense / max * 100}%` }}/></i></button>)}</div><div className="month-bars-readout"><b>{active.label} 2026</b><span>Presupuesto <strong>{clp(active.budget)}</strong></span><span>Ejecutado <strong>{clp(active.expense)}</strong></span><small className={active.expense <= active.budget ? "good-text" : "alert-text"}>Disponible {clp(active.budget - active.expense)}</small></div></div>;
}
...
function AnnualMonthlyBars({ budgets, records }: { budgets: number[]; records: MonthRecord[] }) {
  ...
  useEffect(() => setSelected(Math.max(records.length - 1, 0)), [records.length]);
  return <div className="annual-bars"><div className="annual-bars-legend"><span><i className="annual-plan-key"/> Presupuesto mensual</span><span><i className="annual-actual-key"/> Gasto ejecutado</span><span><i className="annual-pending-key"/> Sin cierre oficial</span></div><div className="annual-bars-plot">{budgets.map((budget, index) => { const month = index + 1; const actual = actualByMonth.get(month); const isSelected = selected === index; return <button key={month} className={isSelected ? "selected" : ""} onClick={() => setSelected(index)} aria-label={`${MONTHS[index]}: presupuesto ${clp(budget)}${actual === undefined ? ", sin cierre oficial" : `, gasto ${clp(actual)}`} `}><span className="annual-bars-columns"><i className="annual-plan-bar" style={{ height: `${budget / max * 100}%` }}/>{actual === undefined ? <i className="annual-pending-bar"/> : <i className="annual-actual-bar" style={{ height: `${actual / max * 100}%` }}/>}</span><em>{MONTHS[index].slice(0, 3)}</em></button>; })}</div><div className="annual-bars-readout"><b>{MONTHS[activeMonth - 1]} 2026</b><span>Presupuesto <strong>{clp(activeBudget)}</strong></span>{activeActual === undefined ? <small>Este mes todavía no tiene un cierre oficial; el gasto no se considera cero.</small> : <><span>Gasto ejecutado <strong>{clp(activeActual)}</strong></span><small className={activeActual <= activeBudget ? "good-text" : "alert-text"}>Disponible {clp(activeBudget - activeActual)}</small></>}</div></div>;
}
...
function CategoryBars({ rows }: { rows: { name: string; annual: number; toDate: number; actual: number }[] }) {
  ...
  return <div className="category-bars"><div>{usable.map((row, index) => <button key={row.name} className={index === selected ? "selected" : ""} onClick={() => setSelected(index)}><span title={row.name}>{row.name}</span><i><em className="category-plan" style={{ width: `${row.toDate / max * 100}%` }}/><em className="category-actual" style={{ width: `${row.actual / max * 100}%` }}/></i></button>)}</div><aside><span>Categoría seleccionada</span><b>{active.name}</b><strong>{clp(active.actual)}</strong><small>Ejecutado de {clp(active.toDate)} presupuestado a la fecha.</small><em className={active.actual <= active.toDate ? "good-text" : "alert-text"}>{pct(active.toDate ? active.actual / active.toDate : 0)} de ejecución</em></aside></div>;
}
